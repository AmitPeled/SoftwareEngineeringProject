...
//				if (userType == UserType.notLogged || gcmDataExecutor.verifyUser(/* userType, */username, password)) {
				switch (requestObject.getQuery()) {
					...
					break;
					case addMap:
						int cityId = (int) listObjectReceived.get(0);
						Map map = (Map) listObjectReceived.get(1);
						File file = (File) listObjectReceived.get(2);
						byte[] fileContent = new byte[(int) file.length()];
						FileInputStream fis = new FileInputStream(file);
						fis.read(fileContent); // read file into bytes[]
						fis.close();
						listToSend.add(gcmDataExecutor.addMapToCity(cityId, map, fileContent));
					break;
