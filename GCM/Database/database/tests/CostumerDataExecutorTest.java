...
public class CostumerDataExecutorTest {
	 ...
	 void getPurchaseHistory() throws SQLException {
		  ...
		  for (int i = 0; i < history.size(); i++) {
			   ...
			   try {
					purchaseHistory = new PurchaseHistory((Date) history.get(i).get(2), (Date) history.get(i).get(5),
					          gcmDataExecutor.getCityById((int) history.get(i).get(1)));
	 ...
	 private List<Report> getAllcitiesReport(Date startDate, Date endDate) throws SQLException {
		  ...
		  for (int i = 0; i < cityIdList.size(); i++) {
			   ...
			   if (!list.isEmpty()) {
					Report report = reportCreateMangerReportOnOneCity((int) cityIdList.get(i).get(0), startDate,
					          endDate);
